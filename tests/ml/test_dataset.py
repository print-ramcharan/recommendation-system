...
@pytest.mark.anyio
async def test_empty_dataset_handling(db_session: AsyncSession):
    # Tests that when there are no clicks in database, empty datasets are returned gracefully
    dataset, user_map, item_map = await build_training_tensors(db_session)
    assert len(dataset) == 0
    assert user_map == {}
    assert item_map == {}

@pytest.mark.anyio
async def test_dataset_generation_and_negative_sampling(db_session: AsyncSession):
    # Seed mock data for verification
    user = User(user_id=8888, username="test_dataset_user", interests={})
    ...
    dataset, user_map, item_map = await build_training_tensors(db_session, negative_ratio=2)
